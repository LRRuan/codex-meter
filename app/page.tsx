...
const COPY = {
  zh: {
    ...
    localTokens: "本地回溯 Token（估算）", notQuota: "非订阅额度；输入含缓存 + 输出", subagentTokens: "Sub-agent Token", includedInScope: "已计入当前范围总量", uncached: "非缓存输入", cached: "缓存输入", output: "输出", reasoning: "推理明细", included: "已包含在输出中",
  ...
  en: {
    ...
    localTokens: "Local reconstructed tokens (estimate)", notQuota: "Not subscription quota; input incl. cache + output", subagentTokens: "Sub-agent tokens", includedInScope: "Included in current scope total", uncached: "Uncached input", cached: "Cached input", output: "Output", reasoning: "Reasoning detail", included: "Included in output",
        ...
        <>
          ...
          <section className="token-panel" aria-labelledby="token-title">
            ...
            <dl className="token-stats">
              ...
              <div className="subagent-stat"><dt>{copy.subagentTokens}</dt><dd>{formatCompact(data?.scope.subagentTokens, 2, locale)}</dd><small>{copy.includedInScope}</small></div>
